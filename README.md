...
3. **Colors** — edit the `[data-theme="..."]` blocks at the top of `style.css`
   to adjust or add color schemes (each just sets `--fg`, `--fg-dim`, `--accent`).
4. **Window title / page title** — edit `<title>` and `.term-title` in
   `index.html`.
5. **No-JS fallback** — the `<noscript>` block in `index.html` is what search
